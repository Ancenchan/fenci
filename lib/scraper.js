...
function cleanPoemHtml(html) {
  ...
  s = s.replace(/<a\s([^>]*)>/gi, (match, attrs) => {
    const parts = [];
    const sm = attrs.match(/style="([^"]*)"/i);   if (sm) parts.push(`style="${sm[1]}"`);
    const hm = attrs.match(/href="([^"]*)"/i);     if (hm) parts.push(`href="${hm[1]}"`);
    const tm = attrs.match(/target="([^"]*)"/i);   if (tm) parts.push(`target="${tm[1]}"`);
    const rm = attrs.match(/rel="([^"]*)"/i);      if (rm) parts.push(`rel="${rm[1]}"`);
  ...
  for (let i = 0; i < 5; i++) {
    s = s.replace(/<span\s([^>]*)>/gi, (match, attrs) => {
      ...
      return sm ? `<span style="${sm[1]}">` : '<span>';
    });
    // 移除空 span
